...
public class PrestamoController {
    ...
    public PrestamoController(PrestamoView view, PrestamoDAO prestamoDao, LibroDAO libroDao, UsuarioDAO usuarioDao) {
        ...
        // Action for creating a new loan
        ...
        // Listener for loan selection in the table
    ...
    private void mostrarDatosSeleccionados() {
        ...
        if (row != -1) {
            ...
            // Obtener el nombre del libro seleccionado desde la tabla
...
// Método para convertir java.util.Date a LocalDate
    ...
    private void cargarLibros() {
        try {
            ...
            for (Libro libro : libros) {
                nombresLibros.add(libro.toString()); // Suponiendo que el método toString() de Libro devuelve el nombre del libro
            }
            DefaultComboBoxModel<String> model = new DefaultComboBoxModel<>(nombresLibros.toArray(new String[0]));
            view.cmbLibros.setModel(model);
        } catch (Exception e) {
    ...
    private void cargarUsuarios() {
        try {
            List<Usuarios> usuarios = usuarioDao.leerUsuario();
            List<String> nombresUsuarios = new ArrayList<>();
            for (Usuarios usuario : usuarios) {
                nombresUsuarios.add(usuario.toString()); // Suponiendo que el método toString() de Usuarios devuelve el nombre del usuario
            }
            DefaultComboBoxModel<String> model = new DefaultComboBoxModel<>(nombresUsuarios.toArray(new String[0]));
            view.cmbUsuarios.setModel(model);
        } catch (Exception e) {
...
public class PrestamoController {
    ...
    public PrestamoController(PrestamoView view, PrestamoDAO dao, UsuarioDAO usuario, LibroDAO libro) {
        this.view = view;
        this.dao = dao;
        this.libro = libro;
        ...
        this.view.btnGestionUsuarios.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                // Abrir la nueva vista UsuarioView
                JFrame usuarioFrame = new JFrame("Gestión de Usuarios");
                UsuarioView usuarioView = new UsuarioView(); // Suponiendo que UsuarioView es la vista que quieres abrir
                usuarioFrame.getContentPane().add(usuarioView);
        ...
        this.view.tablaEventos.getSelectionModel().addListSelectionListener(new ListSelectionListener() {
            @Override
